...
from ._mkl import dot as mkl_dot
from ._mkl import gram as mkl_gram
...
class DeformableShapeModel:
    ...
    Each deformable shape model is defined by the polynomial parameters :math:`\\theta` and the deformation parameters
    :math:`\\xi` (see :ref:`details <pipeline_theory_dsm>`). The polynomial parameters define the polynomial surface
    :math:`f_x^\\top \\theta`, where :math:`f_x` is a second-order polynomial basis function expansion of the image
    point :math:`x`,
    ...
    .. math::
       ...
       A = \\begin{bmatrix}
         a_1 & a_3 \\\\ a_3 & a_2 \\end{bmatrix}, \\qquad b = \\begin{bmatrix} b_1 \\\\ b_2
       \\end{bmatrix}.

    Attributes:
        array: The vector of polynomial and deformation parameters of this model.
        a: The vector :math:`\\theta_{1:3} = (a_1, a_2, a_3)` corresponding to this model.
        b: The vector :math:`\\theta_{4:5} = b = (b_1, b_2)` corresponding to this model.
        c: The parameter :math:`\\theta_{6} = c` of this model.
        ξ: The deformation parameters :math:`\\xi` of this model.
    """
    ...
    def get_model(params):
        ...
        If ``params`` is a :py:class:`DeformableShapeModel` object, then ``params`` is returned. Otherwise, the a new
        :py:class:`DeformableShapeModel` object is instantiated using the given parameters.
        """
    ...
    def s(self, x, smooth_mat):
        ...
        Arguments:
            x: Either a list of coordinates of those image points for which the values of the deformable surface are to
                be computed, or a stack of two 2D arrays corresponding to the pixel coordinates.
        """
    ...
    def create_ellipse(ξ, center, halfaxis1_len, halfaxis2_len, U=None):
        ...
        Attributes:
            ...
            U: An orthonormal matrix whose eigenvectors define the rotation of the ellipse, or `None` if a random
                rotation should be used.
        """
        ev = lambda half_length: (1. / np.square(half_length))  # noqa: E731
        if U is None:
            U = orth(np.random.randn(2, 2))  # random rotation matrix
    ...
    def map_to_image_pixels(self, g, roi, pad=0):
        """
        Transforms the model from the coordinate system of an image region into the coordinate system of the whole
        image.

        Arguments:
            g: An :py:class:`~superdsm.image.Image` object corresponding to the whole image.
            roi: An :py:class:`~superdsm.image.Image` object corresponding to the image region.
            pad: The number of pixels by which the coordinate system of the whole image is padded (once in each
                direction, i.e. twice along each axis). Using a value larger than 0 yields the same result as using an
                image ``g`` padded by the same value.

        Returns:
            The transformed deformable shape model.
        """
        assert pad >= 0 and isinstance(pad, int)
        roi_max_coord = np.array(roi.model.shape) - 1.
...
def _create_gaussian_kernel(sigma, shape=None, shape_multiplier=1):
    if abs(shape_multiplier - 1) > 0 and shape is not None:
        raise ValueError()
    if shape is None:
        shape = [round(1 + sigma * 4 * shape_multiplier)] * 2
    inp = np.zeros(shape)
...
def _convmat(filter_mask, img_shape, row_mask=None, col_mask=None, lock=None):
    ...
    if row_mask is None:
        row_mask = np.ones(img_shape, bool)
    if col_mask is None:
        col_mask = np.ones(img_shape, bool)
    print('.', end='')
...
def _create_masked_smooth_matrix(kernel, mask, subsample=1, lock=None):
    ...
    if (mask.shape <= np.asarray(kernel.shape) // 2).any():
        return None
...
class SmoothMatrixFactory:
    ...
    Arguments:
        smooth_amount: This is :math:`\\sigma_G` described in :ref:`pipeline_theory_dsm`.
    ...
    def get(self, mask, uplift=False):
        ...
        if uplift:
            mat = uplift_smooth_matrix(mat, mask)
...
class Energy:
    ...
    Arguments:
        ...
        smooth_matrix_factory: An object with a ``get`` method which yields the matrix :math:`\\tilde G_\\omega` for
            any image region :math:`\\omega` (represented as a binary mask and passed as a parameter).
    """
    ...
    def _update_maps(self, params):
        if self.p is not None and all(self.p.array == params.array):
            return
    ...
    def __call__(self, params):
        ...
        The parameters are represented by ``params`` so that ``params[:6]`` corresponds to the polynomial parameters
        :math:`\\theta` and ``params[6:]`` corresponds to the deformation parameters :math:`\\xi`.
        """
    ...
    def grad(self, params):
        ...
        if self.smooth_mat.shape[1] > 0:
            ...
            # The next line performs column-wise sumation, but when paraphrased as:
            #
    ...
    def hessian(self, params):
        ...
        The parameters are represented by ``params`` so that ``params[:6]`` corresponds to the polynomial parameters
        :math:`\\theta` and ``params[6:]`` corresponds to the deformation parameters :math:`\\xi`. The Hessian matrix
        is returned as a sparse block matrix.
        """
        params = DeformableShapeModel.get_model(params)
        self._update_maps(params)
        self._update_theta()
...
class _Cache:
    ...
    def __init__(self, size, getter, equality=None):
        ...
        elif isinstance(equality, str):
            equality = eval(equality)
...
class CP:
    ...
    Arguments:
        energy: The convex energy function to be minimized, so that ``energy(p)`` corresponds to the value of
            :math:`\\psi(p)`, ``energy.grad(p)`` corresponds to the gradient vector :math:`\\nabla\\psi(p)`, and
            ``energy.hessian(p)`` corresponds to the Hessian matrix :math:`\\nabla^2 \\psi(p)`.
        params0: Parameters vector used for initialization.
        scale: Fixed factor used to slightly improve numerical stabilities.
        cachesize: The maximum number of entries used for caching the values of the ``energy`` function, the gradient,
            and the Hessian.
        cachetest: The test function to be used for cache testing. If `None`, then ``numpy.array_equal`` will be used.
        timeout: The maximum run time of the :py:meth:`~.solve` method (in seconds). Convex programming will be
            interrupted by raising a :py:class:`~TimeoutError` if it takes longer than that. If this is set to `None`,
            the run time is not limited.
    """

    CHECK_NUMBERS = False
    """
    Performs additional assertions when set to `True` which are useful for debugging but might increase the overall run
    time (sparse matrices need to be converted to dense).
    """

    def __init__(self, energy, params0, scale=1, cachesize=0, cachetest=None, timeout=None):
        self.params0 = params0
        self.gradient = _Cache(
            cachesize,
            lambda p: (scale * energy(p), cvxopt.matrix(scale * energy.grad(p)).T),
            equality=cachetest,
        )
