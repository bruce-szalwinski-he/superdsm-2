...
class BaseObject:
    ...
    The attributes :py:attr:`~.fg_offset` and :py:attr:`~.fg_fragment` are initialized with `None`, indicating that
    ...
    def fill_foreground(self, out, value=True):
        ...
        The foreground fragment is written into the image ``out``, which must be an object of ``numpy.ndarray`` type.
        Image points corresponding to the segmentation mask will be set to ``value``.

        The method requires that :py:attr:`~.fg_offset` and :py:attr:`~.fg_fragment` have been computed before (see the
        :py:meth:`~compute_objects` function).
        ...
        sel = np.s_[
            self.fg_offset[0] : self.fg_offset[0] + self.fg_fragment.shape[0],
            self.fg_offset[1] : self.fg_offset[1] + self.fg_fragment.shape[1],
        ]
...
class Object(BaseObject):
    ...
    Each object corresponds to a realization of the set :math:`X` in :ref:`Kostrykin and Rohr (TPAMI 2023 <references>`,
    Section 3). It also represents a segmented object after it has been passed to the :py:meth:`compute_objects`
    function.

    Attributes:
        footprint: Set of integer labels that identify the atomic image regions, which the object represents.
        energy: The value of the set energy function :math:`\\nu(X)` (see
            :ref:`pipeline_theory_jointsegandclustersplit`).
        on_boundary: `True` if this object intersects the image boundary.
        is_optimal: `True` if optimization of :py:attr:`~.energy` was successful.
        processing_time: How long the computation of the attributes took (in seconds).

    The attributes :py:attr:`~.energy`, :py:attr:`~.on_boundary`, :py:attr:`~.is_optimal`, :py:attr:`~.processing_time`
    are initialized with ``nan``, which indicates that the values have not been computed yet, i.e. the object was not
    passed to the :py:meth:`~compute_objects` function yet.

    Possible reasons for :py:attr:`~.is_optimal` being `False` include the rare cases of numerical issues during
    optimization as well as regions of the size of a single pixel.
    """
    ...
    def get_mask(self, atoms):
        ...
        Arguments:
            atoms: Integer-valued image representing the universe of atomic image regions (each atomic image region has
                a unique label, which is the integer value).

        Returns:
            Binary image corresponding to :math:`\\tilde\\omega(X) = \\bigcup X` in the paper, where each object of
            this class is a realization of the set :math:`X` (see :ref:`Kostrykin and Rohr, TPAMI 2023 <references>`,
            Section 3).
        """
    ...
    def get_cvxprog_region(self, y, atoms, background_margin):
        ...
        Arguments:
            y: Object of :py:class:`~.image.Image` class, corresponding to the offset image intensities.
            atoms: Integer-valued image representing the universe of atomic image regions (each atomic image region has
                a unique label, which is the integer value).
            background_margin: Governs the amount of image background included in the obtained image region. This is
                the width of the "stripe" of background retained around each connected foreground region (in pixels,
                intersected with the image region determined by the :py:meth:`~.get_mask` method).

        Returns:
            Image region corresponds to :math:`\\tilde\\omega'(X)` in
            :ref:`Kostrykin and Rohr (TPAMI 2023 <references>`) see Supplemental Material 6), where each object of this
            class is a realization of the set :math:`X` (see :ref:`Kostrykin and Rohr, TPAMI 2023 <references>`,
            Section 3). The image region is represented by an object of :py:class:`~.image.Image` type.
        """
...
def _compute_object(y, atoms, x_map, object, dsm_cfg, smooth_mat_allocation_lock):
    ...
    for infoline in (
        'y.mask.sum()',
        'region.mask.sum()',
        'np.logical_and(region.model > 0, region.mask).sum()',
        'cp_kwargs',
    ):
    ...
    # Otherwise, perform model fitting
        ...
        object.on_boundary = any(
            (
                padded_foreground[0].any(),
                padded_foreground[-1].any(),
                padded_foreground[:, 0].any(),
                padded_foreground[:, -1].any(),
            )
        )
...
def compute_objects(objects, y, atoms, dsm_cfg, log_root_dir, status_line=DEFAULT_COMPUTING_STATUS_LINE, status=None):
    ...
    The computation concerns the attributes :py:attr:`~Object.energy`, :py:attr:`~Object.on_boundary`,
    :py:attr:`~Object.is_optimal`, :py:attr:`~Object.processing_time`, :py:attr:`~BaseObject.fg_fragment`,
    :py:attr:`~BaseObject.fg_offset` of the objects.

    Attributes:
        objects: List of objects for which the above mentioned attributes are to be computed.
        y: An :py:class:`~.image.Image` object corresponding to the offset image intensities.
        ...
        dsm_cfg: Dictionary of hyperparameters defined in the :py:class:`~superdsm.dsmcfg.DSM_Config` stage (without
            the leading ``dsm/`` namespace prefix).
        log_root_dir: Path of directory where log files will be written, or `None` if no log files should be written.
        status_line: Tuple ``(s1, s2)``, where ``s1`` is the line of text to be written while objects are being
            computed, and ``s2`` is the line of text to be written when finished.
    ...
    with SystemSemaphore('smooth-matrix-allocation', smooth_mat_max_allocations) as smooth_mat_allocation_lock:
        ...
        for ret_idx, ret in enumerate(
            _compute_objects(objects, y, atoms, x_map, smooth_mat_allocation_lock, dsm_cfg, log_root_dir),
        ):
            objects[ret[0]].set(ret[1])
            repype.status.update(
                status,
                f'{status_line[0]}... {ret_idx + 1} / {len(objects)} ({fallbacks}x fallback)',
                intermediate=True,
            )
            if ret[2]:
                fallbacks += 1
...
def _compute_objects(objects, y, atoms, x_map, lock, dsm_cfg, log_root_dir):
    if _compute_objects._DEBUG:  # run serially
        ...
        futures    = [
            _ray_compute_object_logged.remote(
                log_root_dir,
                obj_idx,
                y_id,
                atoms_id,
        ...
        for ret in get_ray_1by1(futures):
            yield ret
...
def _compute_elliptical_solution(J_elliptical, CP_params):
    ...
    except:  # noqa: E722  # e.g., `Rank(A) < p or Rank([H(x); A; Df(x); G]) < n` error which happens rarely
        traceback.print_exc()
...
def cvxprog(
        ...
        smooth_amount,
        smooth_subsample,
        gaussian_shape_multiplier,
        smooth_mat_allocation_lock,
        smooth_mat_dtype,
        init=None,
        cachesize=0,
        cachetest=None,
    ...
    Performs convex programming in an image region :math:`X` to determine the value of the set energy function
    :math:`\\nu(X)` and the optimal parameters :math:`\\theta` and :math:`\\xi` (see :ref:`pipeline_theory_cvxprog` and
    :ref:`pipeline_theory_jointsegandclustersplit`).

    Arguments:
        region: An :py:class:`~image.Image` object corresponding to the image region :math:`X`.
        smooth_mat_allocation_lock: A critical section lock used for allocation of the matrix :math:`\\tilde G_\\omega`.

    The other parameters correspond to the hyperparameters defined in the :py:class:`~superdsm.dsmcfg.DSM_Config` stage
    (without the leading ``dsm/`` namespace prefix).

    Returns:
        A tuple with the following components:

            * The value of the set energy function :math:`\\nu(X)`.
            * An instance of the :py:class:`~dsm.DeformableShapeModel` class which represents the optimal parameters
              :math:`\\theta` and :math:`\\xi`.
            * A status indicator string, where ``optimal`` indicats that convex programming was successful and
              ``fallback`` indicates that convex programming failed for the deformable shape model and the
              initialization is used instead.
    """
    _print_heading('initializing')
    smooth_matrix_factory = SmoothMatrixFactory(
        smooth_amount,
        gaussian_shape_multiplier,
        smooth_subsample,
        smooth_mat_allocation_lock,
    ...
    CP_params = {
        'cachesize': cachesize,
        'cachetest': cachetest,
        'scale': scale / J.smooth_mat.shape[0],
        'timeout': cp_timeout,
    }
    ...
    try:
        ...
        if solution_info['status'] == 'unknown' and J(solution) > J(params):
            status = 'fallback'  # numerical difficulties lead to a very bad solution, fall back to the ellipse solution
        else:
    ...
    if status == 'fallback':
        _print_heading(
            f'DSM failed: falling back to {"elliptical result" if init == "elliptical" else "initialization"}',
        )
        solution = params
