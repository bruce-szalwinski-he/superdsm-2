...
class DSM_Config(repype.stage.Stage):
    ...
    The purpose of this stage is to provide the hyperparameters from the ``dsm`` namespace as the output ``dsm_cfg``,
    which is a dictionary of the hyperparameters without the leading ``dsm/`` namespace prefix. This enables any stage
    to access the DSM-related hyperparameters, like the :py:class:`~.c2freganal.C2F_RegionAnalysis` and
    :py:class:`~.globalenergymin.GlobalEnergyMinimization` stages, without having to access the ``dsm`` hyperparameter
    namespace. Refer to :ref:`pipeline_inputs_and_outputs` for more information on the available inputs and outputs.
    ...
    ``dsm/cachesize``
        The maximum number of entries used for caching during convex programming. This concerns invocations of the
        callback function ``F`` used by the `cvxopt solver for nonlinear problems
        <https://cvxopt.org/userguide/solvers.html#problems-with-nonlinear-objectives>`_. Defaults to 1.

    ``dsm/cachetest``
        The test function to be used for cache testing. If `None`, then ``numpy.array_equal`` will be used. Using other
        functions like ``numpy.allclose`` has shown to introduce numerical instabilities. Defaults to `None`.

    ``dsm/init``
        Either a function or a string. If this is function, then it will be called to determine the initialization, and
        the dimension of the vector :math:`\\xi` will be passed as a parameter. If this is a string, then the
        initialization corresponds to the result of convex programming using elliptical models (if set to
        ``elliptical``, see Supplemental Material 6 of :ref:`Kostrykin and Rohr, TPAMI 2023 <references>`) or a zeros
        vector of is used (otherwise). Defaults to ``elliptical``.

    ``dsm/smooth_amount``
        Corresponds to :math:`\\sigma_G` described in :ref:`pipeline_theory_dsm`. Defaults to 10, or to
        ``AF_smooth_amount × scale`` if computed automatically (forced to :math:`\\geq 4` and ``AF_smooth_amount``
        defaults to 0.2).
    ...
    ``dsm/gaussian_shape_multiplier``
        The Gaussian function with standard deviation :math:`\\sigma_G` used to construct the block Toeplitz matrix
        :math:`G_\\omega` is cut off after :math:`4 \\sigma_G` multiplied by this value (see
        :ref:`pipeline_theory_dsm`). Defaults to 2.
    ...
    ``dsm/smooth_mat_max_allocations``
        Maximum number of simultaneous allocation of the matrix :math:`\\tilde G_\\omega` during parallel processing
        (see Section 3.3 of :ref:`Kostrykin and Rohr, TPAMI 2023 <references>`, each allocation might require a
        considerable amount of system memory).
    ...
    ``dsm/cp_timeout``
        ...
        interrupted if it takes longer than that (the :py:meth:`~superdsm.objects.cvxprog` function will report the
        status ``fallback`` in this case). If this is set to `None`, the run time is not limited. Defaults to 300 (i.e.
        5 minutes).
    """
