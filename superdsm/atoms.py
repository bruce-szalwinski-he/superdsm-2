...
class AtomAdjacencyGraph:
    ...
    def get_edge_lines(self, accept='all', reduce=True):
        ...
        Arguments:
            accept: Must be either ``all`` or a callable. If ``all`` is used, all edges of the graph are included.
                Otherwise, an edge ``(i,j)`` is included only if ``accept(i)`` and ``accept(j)`` evaluate to `True`,
                where ``i`` and ``j`` are the labels of two adjacent atomic image regions.
            reduce: If `True`, then an edge ``(i,j)`` is included only if ``i > j``. Otherwise, both edges ``(i,j)``
        ...
        """
        if isinstance(accept, str) and accept == 'all':
            accept = lambda atom_label: True  # noqa: E731
        assert callable(accept), f'Not a callable: {str(accept)}'
        lines = []
        for l in self.atom_labels:  # noqa: E741
