...
def render_adjacencies(
        data,
        normalize_img=True,
        edge_thickness=3,
        endpoint_radius=5,
        endpoint_edge_thickness=2,
        edge_color=(1, 0, 0),
        endpoint_color=(1, 0, 0),
        endpoint_edge_color=(0, 0, 0),
        override_img=None,
    ):
    """
    Returns a visualization of the adjacency graph (see :ref:`pipeline_theory_c2freganal`).

    By default, the adjacency graph is rendered on top of the contrast-enhanced raw image itensities. Contrast
    enhancement is performed using the :py:meth:`normalize_image` function.

    Arguments:
        data: The pipeline data object.
        normalize_img: `True` if contrast-enhancement should be performed and `False` otherwise. Only used if
            ``override_img`` is `None`.
        ...
        override_img: The image on top of which the adjacency graph is to be rendered. If `None`, the
            (contrast-enhanced) raw image itensities will be used.
    ...
    for line in lines:
        ...
        for i in range(3):
            img[:, :, i][line_mask] = (line_vals) * edge_color[i]
    for endpoint in data['seeds']:
...
def render_ymap(data, clim=None, cmap='bwr'):
    """
    Returns a visualization of the offset image intensities :math:`Y_\\omega|_{\\omega = \\Omega}` (see
    :py:ref:`pipeline_theory_cvxprog`).

    Arguments:
        data: The pipeline data object.
        clim: Tuple of the form ``(cmin, cmax)``, where ``cmin`` and ``cmax`` are used for intensity clipping. The
            limits ``cmin`` and ``cmax`` are chosen automatically if ``clim`` is set to `None`.
        cmap: Name of the color map to use for encoding the offset image intensities (see
            `the list <https://matplotlib.org/stable/tutorials/colors/colormaps.html>`_).
    ...
    y /= y.max()
    ymap = cmap(y)[1:]
    if ymap.ndim == 3 and ymap.shape[2] == 4:
        ymap = ymap[:, :, :3]
    return ymap
...
def normalize_image(img, spread=1, ret_minmax=False):
    ...
    Arguments:
        ...
        ret_minmax: `True` if the clipped image intensities should be returned and `False` otherwise.

    Returns:
        The contrast-enhanced image if ``ret_minmax`` is `False`, and a tuple of the structure ``(img, minval, maxval)``
        if ``ret_minmax`` is `True`, where ``img`` is the contrast-enhanced image, and ``minval`` and ``maxval`` are
        the clipped image intensities, respectively.
    ...
    if not np.allclose(img.std(), 0):
        minval = max([img.min(), img.mean() - spread * img.std()])
        maxval = min([img.max(), img.mean() + spread * img.std()])
...
def render_atoms(
    ...
    Arguments:
        ...
        override_img: The image on top of which the borders of the atomic image regions are to be rendered. If `None`,
            the (contrast-enhanced) raw image itensities will be used.
    ...
    return render_regions_over_image(
        img / img.max(),
        data['atoms'],
        background_label=0,
        bg=discarded_color,
...
def render_foreground_clusters(
        data,
        normalize_img=True,
        discarded_color=(0.3, 1, 0.3, 0.1),
        border_radius=2,
    ...
    """
    Returns a visualization of regions of possibly clustered objects (see
    :ref:`pipeline_theory_jointsegandclustersplit`).
    ...
    Arguments:
        ...
        override_img: The image on top of which the borders of the regions of possibly clustered objects are to be
            rendered. If `None`, the (contrast-enhanced) raw image itensities will be used.
...
def rasterize_regions(regions, background_label=None, radius=3):
    ...
    Arguments:
        regions: Integer-valued image (object of ``numpy.ndarray`` type) corresponding to the labels of different image
            regions.
    ...
    Returns:
        Tuple of the form ``(borders, background)``, where ``borders`` is a binary mask of the borders of the image
        regions, and ``background`` is a binary mask of the union of those regions corresponding to the
        ``background_label``, if this is not `None`. Otherwise, ``background`` is an binary mask filled with `False`
        values.
    """
    ...
    for i in range(regions.max() + 1):
        ...
        if i == background_label:
            background = interior.astype(bool)
...
def render_regions_over_image(img, regions, background_label=None, color=(0, 1, 0), bg=(0.6, 1, 0.6, 0.3), **kwargs):
    ...
    Arguments:
        ...
        bg: The color of the image regions corresponding to the image background (RGBA). Only used if
            ``background_label`` is not `None`.
        kwargs: Keyword arguments passed to the :py:meth:`~.rasterize_regions` function.
    ...
    for i in range(3):
        result[:, :, i][borders] = color[i]
    for i in range(3):
        result[background, i] = bg[i] * bg[3] + result[background, i] * (1 - bg[3])
...
class ContourPaint:
    ...
    Arguments:
        fg_mask: Binary mask of the image foreground. Any contour never overlaps the image foreground except of those
            image regions corresponding to the contoured object itself.
    ...
    def get_contour_mask(self, mask):
        ...
        if self.where == 'center':
            contour = np.logical_xor(
                morphology.binary_erosion(mask, self.selem),
                morphology.binary_dilation(mask, self.selem),
            )
...
def render_result_over_image(
        data,
        objects='postprocessed_objects',
        merge_overlap_threshold=np.inf,
        normalize_img=True,
        border_width=6,
    ...
    Arguments:
        data: The pipeline data object.
        objects: Either the name of the output which is to be treated as the segmentation result (see
            :ref:`pipeline_inputs_and_outputs`), or a list of :py:class:`~superdsm.objects.BaseObject` instances.
        merge_overlap_threshold: Any pair of two objects with an overlap larger than this threshold will be merged into
            a single object.
        normalize_img: `True` if contrast-enhancement should be performed and `False` otherwise. Only used if
            ``override_img`` is `None`.
        border_width: The width of the contour to be drawn around the segmented objects.
        border_position: The position of the contour to be drawn around the segmented objects (``inner``, ``center``,
            or ``outer``).
        override_img: The image on top of which the contours of the segmented objects are to be rendered. If `None`,
            the (contrast-enhanced) raw image itensities will be used.
        color: The color of the contour to be drawn around the segmented objects (``r`` for red, ``g`` for green, ``b``
            for blue, ``y`` for yellow, ``t`` for teal, or ``w`` for white).
    ...
    for l in set(seg_objects.flatten()) - {0}:  # noqa: E741
        seg_bnd = cp.get_contour_mask(seg_objects == l)
        colorchannels = COLORMAP[color]
        for i in range(3):
            im_seg[seg_bnd, i] = (1 if i in colorchannels else 0)
...
def rasterize_objects(data, objects, dilate=0):
    ...
    Arguments:
        data: The pipeline data object.
        objects: Either the name of the output which is to be rasterized (see :ref:`pipeline_inputs_and_outputs`), or
            a list of :py:class:`~superdsm.objects.BaseObject` instances.
        dilate: Dilates the segmentation mask of each object by this value, or erodes if negative.
    """
    if isinstance(objects, str):
        objects = [c for c in data[objects]]

    if dilate == 0:
        dilation, erosion = None, None
    else:
    ...
    for foreground in render_objects_foregrounds(data['g_raw'].shape, objects):
        if dilate > 0:
            foreground = dilation(foreground, morphology.disk( dilate))
        elif dilate < 0:
            foreground =  erosion(foreground, morphology.disk(-dilate))
        if foreground.any():
            yield foreground.copy()
...
def rasterize_labels(
    ...
    Arguments:
        data: The pipeline data object.
        objects: Either the name of the output which is to be rasterized (see :ref:`pipeline_inputs_and_outputs`), or a
            list of :py:class:`~superdsm.objects.BaseObject` instances.
        merge_overlap_threshold: Any pair of two objects with an overlap larger than this threshold will be merged into
            a single object.
        dilate: Dilates the segmentation mask of each object by this value, or erodes if negative.
        background_label: The label which is to be assigned to the image background. Must be non-positive.

    Returns: An object of type ``numpy.ndarray`` corresponding the uniquely labeled segmentation masks.
    """
    ...
    for label, obj_idx in zip(labels, range(len(objects))):
        obj_indices_by_label[label] = [obj_idx]
    ...
    if len(objects) > 0:
        ...
        for l, obj in enumerate(objects, 1):  # noqa: E741
            result[obj] = l
...
def shuffle_labels(labels, bg_label=None, seed=None):
    ...
    for l in label_map.keys():  # noqa: E741
        cc = (labels == l)
...
def colorize_labels(labels, bg_label=0, cmap='gist_rainbow', bg_color=(0, 0, 0), shuffle=None):
    ...
    Arguments:
        ...
        cmap: The colormap used to colorize the remaining labels (see
            `the list <https://matplotlib.org/stable/tutorials/colors/colormaps.html>`_).
        bg_color: The color used to represent the image regions with label ``bg_label`` (RGB).
        shuffle: If not `None`, then used as ``seed`` to shuffle the labels before colorization (see
            :py:meth:`~.shuffle_labels`), and not used otherwise.
